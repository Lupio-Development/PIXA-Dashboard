...
export const AddItem = (props: any) => {
    ...
    const { productAddItemData, totalPendingData } = useSelector((state: RootStore) => state.productRequest);
    ...
    const ManageSellerRequestData = [
        ...
        {
            Header: "Product",
            accessor: "mainImage", // Ensure an accessor if needed for sorting or filtering
            Cell: ({ row }) => (
              <div className="d-flex justify-content-center align-items-center">
                {/* Image Section */}
                <div
...

        {
            Header: "Reason",
            body: "reason",
            Cell: ({ row }) => <span>{row?.creationCause || "-"}</span>,
        },
