...
const WithdrawSetting = () => {
  ...
  const [minWithdrawalRequestedCoin, setMinWithdrawalRequestedCoin] =
    ...
    const [minimumPayoutAmount , setMinimumPayoutAmount] =
    useState();
  ...
  useEffect(() => {
    ...
    setMinimumPayoutAmount(settingData?.minimumPayoutAmount)
  }, [settingData]);
  ...
  const handleSubmit = () => {
    ...
    const minWithdrawalRequestedCoinValueForSeller = parseInt(
      minimumPayoutAmount
    );

    if (minWithdrawalRequestedCoinValue <= 0 || minWithdrawalRequestedCoinValueForSeller <=0) {
      let error: any = {};
      ...
      if (minimumPayoutAmount === "")
        if (minWithdrawalRequestedCoinValueForSeller <= 0)
          error.minWithdrawalRequestedCoin = "Amount Invalid !";
            ...
            <div className="withdrawal-box payment-setting ">
              ...
              <div className="row">
                <div className="col-12 withdrawal-input mt-1">
                  <Input
                    label={"Minimum Withdrawal Coin (User)"}
                    ...
                    onChange={(e) => {
                      ...
                      if (!e.target.value) {
                        return setError({
                          ...error,
                          minWithdrawalRequestedCoin: `User Amount Is Required`,
                        });
                      } else {
                        return setError({
                          ...error,
                  ...
                  />
                  <h6>Minimum withdrawal coin the users can request</h6>
                </div>

                <div className="col-12 withdrawal-input mt-1">
                  <Input
                    label={"Minimum Withdrawal Amount (Seller)"}
                    name={"minWithdrawalRequestedCoin"}
                    type={"number"}
                    value={minimumPayoutAmount}
                    errorMessage={
                      error.minimumPayoutAmount &&
                      error.minimumPayoutAmount
                    }
                    placeholder={"Enter Amount"}
                    onChange={(e) => {
                      setMinimumPayoutAmount(e.target.value);
                      if (!e.target.value) {
                        return setError({
                          ...error,
                          minimumPayoutAmount: `Seller Amount Is Required`,
                        });
                      } else {
                        return setError({
                          ...error,
                  ...
                  />
                  <h6>Minimum withdrawal amount the seller can request</h6>
                </div>
              </div>
            </div>
          </div>
