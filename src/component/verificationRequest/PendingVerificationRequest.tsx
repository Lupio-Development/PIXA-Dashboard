...
const PendingVerificationRequest = () => {
  ...
  const postReportTable = [
...

    {
      ...
      Cell: ({ row }: { row: any }) => (
        <>
          <img
            src={ row?.userId?.image || maleImage.src }
            width="60px"
            height="60px"
...

    {
      ...
      Cell: ({ row, index }: { row: any; index: number }) => (
        <img
          src={row?.profileSelfie || noImage.src}
          width="60px"
          height="60px"
  ...
  const handleSolved = (id: any) => {
      const data = warningForAccept("verification");
       data
           .then((res) => {
             if (res) {
          
                  dispatch(verificationRequestAccept(id));
