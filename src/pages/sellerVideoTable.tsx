...
import SellerVideo from "@/component/sellerVideo/sellerVideo";
import FakeSellerVideo from "@/component/sellerVideo/FakeSellerVideo";

const sellerVideoTable = (props) => {
  ...
  return (
    <div className="userPage channelPage pt-0">
      ...
      {multiButtonSelect === "Real Video" && (
        <SellerVideo startDate={startDate} endDate={endDate} />
      )}
      {multiButtonSelect === "Fake Video" && (
        <FakeSellerVideo startDate={startDate} endDate={endDate} />
      )}
    </div>
  );
};

sellerVideoTable.getLayout = function getLayout(page) {
  return <RootLayout>{page}</RootLayout>;
};

export default sellerVideoTable;
