...
export const getAdminEarningHistory: any = createAsyncThunk(
    "admin/order/adminEarningHistory",
    async (payload: AllUsersPayload | undefined) => {
        return apiInstanceFetch.get(`admin/order/adminEarningHistory?startDate=${payload?.startDate}&endDate=${payload?.endDate}&start=${payload?.page}&limit=${payload?.size}`);
    }
);
...
const adminEaningSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(getAdminEarningHistory.pending, (state, action: PayloadAction<any>) => {
            state.isSkeleton = true;
            state.isLoading = true
        });
        builder.addCase(
            getAdminEarningHistory.fulfilled,
            (state, action: PayloadAction<any>) => {
                
                state.isLoading = false;
                state.adminEarningHistory = action.payload.data;
                state.total = action.payload.total;
                state.totalAdminEarning = action.payload.totalEarnings;
                state.isLoading = false

            }
        );
        builder.addCase(getAdminEarningHistory.rejected, (state, action: PayloadAction<any>) => {
            state.isLoading = false;
        });
