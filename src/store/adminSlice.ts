...
export const uploadMultipleImage : any = createAsyncThunk(
  "admin/uploadImage/add",
  async (payload) => {
      const response = await axios.put(
          `admin/file/upload_multiple_files`,
          payload
      );
      return response;
  }
  ...
  extraReducers: (builder: any) => {
    ...
    builder.addCase(
      ...
      (state: any, action: PayloadAction<any>) => {
        ...
        if (action.payload && action.payload?.data?.status === true) {
    ...
    builder.addCase(
      uploadMultipleImage.pending,
      (state: any, action: PayloadAction<any>) => {
        state.isLoading = true;
      }
    );
    builder.addCase(
      uploadMultipleImage.fulfilled,
      (state: any, action: PayloadAction<any>) => {
    ...
    builder.addCase(
     ...
     builder.addCase(
       adminProfileUpdated.fulfilled,
       (state: any, action: PayloadAction<any>) => {
         state.isLoading = false;
         const currentAdmin = state.admin;
         const emailUpdated =
           action.payload?.data?.data?.email !== currentAdmin?.email;
         const passwordUpdated =
           action.payload?.data?.data?.password !== currentAdmin?.password;
