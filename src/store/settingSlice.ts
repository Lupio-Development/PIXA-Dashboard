...
export const getDocumentTypeSetting = createAsyncThunk(
  "admin/documentType/fetchDocumentTypes",
  async (payload: settingPayload | undefined) => {
    return apiInstanceFetch.get(`admin/documentType/fetchDocumentTypes`);
  }
);
...
export const createDocumentTypetSetting: any = createAsyncThunk(
  "admin/documentType/createDocumentType?title=PassPort",
  async (payload: any) => {
    return axios.post(`admin/documentType/createDocumentType?title=${payload?.title}`);
  }
);
...
export const updateDocuemtTypeSetting = createAsyncThunk(
  "admin/documentType/modifyDocumentType?title=PassPort&documentTypeId",
  async (payload: any) => {
    return axios.patch(
      `admin/documentType/modifyDocumentType?documentTypeId=${payload?.documentTypeId}&title=${payload?.title}`,
    );
  }
);
...
export const deleteDocumentTypeSetting = createAsyncThunk(
  "admin/reportReason/deleteDocumentType",
  async (payload: any) => {
    return axios.delete(`admin/documentType/removeDocumentType?documentTypeId=${payload}`);
  }
);
...
const settingReducer = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      getDocumentTypeSetting.pending,
      (state, action: PayloadAction<any>) => {
        state.isLoading = true;
      }
    );
    ...
    builder.addCase(
      getDocumentTypeSetting.fulfilled,
      (state, action: PayloadAction<any, string, { arg: settingPayload }>) => {
    ...
    builder.addCase(
      getDocumentTypeSetting.rejected,
      (state, action: PayloadAction<any>) => {
        state.isLoading = false;
      }
    );
    ...
    builder.addCase(createDocumentTypetSetting.pending, (state) => {
      state.isLoading = true;
    });

    builder.addCase(
      createDocumentTypetSetting.fulfilled,
      (state, action: PayloadAction<any>) => {
        
        state.isLoading = false;
        if (action.payload.data.status === true) {
          state.documentType.unshift(action.payload.data.data);
          setToast("success", `Document Type Created`);
        }
      }
    );

    builder.addCase(createDocumentTypetSetting.rejected, (state) => {
      state.isLoading = false;
    });
    ...
    builder.addCase(updateDocuemtTypeSetting.pending, (state) => {
      state.isLoading = true;
    });

    builder.addCase(
      updateDocuemtTypeSetting.fulfilled,
      (state, action: PayloadAction<any, string, { arg: settingPayload }>) => {
        state.isLoading = false;
        if (action.payload.data.status === true) {
          ...
          setToast("success", ` Document Type Update Successfully`);
        }else {
    ...
    builder.addCase(updateDocuemtTypeSetting.rejected, (state) => {
      state.isLoading = false;
    });
    ...
    builder.addCase(
      deleteDocumentTypeSetting.pending,
      (state, action: PayloadAction<any>) => {
        state.isLoading = true;
      }
    );

    builder.addCase(deleteDocumentTypeSetting.fulfilled, (state, action: any) => {
      if (action?.payload?.status) {
        state.documentType = state.documentType.filter(
          (documentType: any) => documentType._id !== action?.meta?.arg
        );

        setToast("success", "Document Type Delete Successfully");
      } else {
        setToast("error", action.payload.data?.message);
      }
      state.isLoading = false;
    ...
    builder.addCase(deleteDocumentTypeSetting.rejected, (state, action) => {
      state.isLoading = false;
    });
